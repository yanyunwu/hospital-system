...
export class BookingController {
  ...
  async getBookingList(
    ...
    const [data, count] = await this.bookingService.getBookingList(
      skip,
      take,
      options as Booking,
    );
  ...
  @Post('/delBooking')
  delBooking(@Body() body: { ids: number[] }) {
  ...
  async getBookingDateList(
    ...
    const [data, count] = await this.bookingService.getBookingDateList(
      skip,
      take,
      options as BookingDate,
      parseInt(bookingId),
    );
  ...
  @Post('/delBookingDate')
  delBookingDate(@Body() body: { ids: number[] }) {
  ...
  async getBookingDateRecordList(
    ...
    const { skip = 0, take = 20, ...options } = qurey;
    const [data, count] = await this.bookingService.getBookingDateRecordList(
      skip,
      take,
      options as BookingDateRecord,
    );
  ...
  async addBookingDateRecord(
    @Body() body: BookingDateRecord & { bookingDateId: number },
    @Req() req: Request,
  ) {
    const userInfo = req['user'];
    const { bookingDateId, ...rest } = body;
    return this.bookingService.addBookingDateRecord(
      bookingDateId,
      parseInt(userInfo.userId),
      rest,
    );
  ...
  delBookingDateRecord(@Body() body: { ids: number[] }) {
