...
export class BookingService {
    ...
    // date
    async getBookingDateList(skip?: number, take?: number, options?: BookingDate, bookingId?: number): Promise<[Array<BookingDate & { key: number }>, number]> {
    ...
    async addBookingDate(bookingId: number, book: BookingDate) {
        const thebook = await this.bookingRepository.findOne({where: { id:bookingId }})
    ...
    async delBookingDate(ids: number[]) {
     ...
     // record
     async getBookingDateRecordList(skip?: number, take?: number, options?: BookingDateRecord): Promise<[Array<BookingDateRecord & { key: number }>, number]> {
    ...
    async addBookingDateRecord(bookingDateId: number, userId: number, book: BookingDateRecord) {
        const bookDate = await this.bookingDateRepository.findOne({where: {id: bookingDateId}})
